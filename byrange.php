...
<center>
  LOWER RANGE: <input type="number" name="range">
  HIGHER RABGE: <input type="number" name="range1">   
  <br>
  <br>
  <input type="submit" name="range3" value="GET SELECTED VALUE" class="btn btn-primary"></center>
  <br>
</form>
    <?php
            ...
            <select name="hall1" >';
                $k=$_POST['range'];
                $k1=$_POST['range1'];
                $result ="SELECT hall_name FROM hall WHERE hall_charge >= '$k' AND hall_charge <= '$k1'  ";
    ...
    while($row = $a-> fetch_assoc()) {
     ...
     echo '<input name="hh1" type="submit" value="get you data" class="btn btn-primary">';
        echo '</form></div>';
            }
            ?>
            <?php
    if(isset($_POST['hh1'])){
$selected_val = $_POST['hall1']; 
    #echo "hell0";
